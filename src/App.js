...
function App() {
  ...
  const [tipoAuto, setTipoAuto] = useState('todos'); 
  ...
  const productosFiltrados = product.filter(auto => tipoAuto === 'todos' || auto.category === tipoAuto); 

  return (
    <div>
      <NavBar count={cart.length}
        handleShow={handleShow}
        setTipoAuto={setTipoAuto} ></NavBar> 

      {
        showCart ?
          <CartList cart={cart} ></CartList> :
          <ProductList product={productosFiltrados} addToCart={addToCart} ></ProductList> 
      }
    </div>
  );
}
